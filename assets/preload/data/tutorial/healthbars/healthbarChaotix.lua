...
local width = 667 -- default: 593
local height = 25 -- default: 11

local bar_offset_x = 220 -- default: 0
local bar_offset_y = 5 -- default: 0

local border_color = "000000"
local border_thickness = 4 -- default: 4

local override_colors = true -- default: false
local override_p1_color = override_colors and "00D0FF" or nil
local override_p2_color = override_colors and "FF283B" or nil

local p1_offset_x = -26 -- default: -26
local p1_offset_y = -5 -- default: 10
local p2_offset_x = 26 -- default: 26
local p2_offset_y = -5 -- default: 10
...
function onCreatePost()
	...
	makeLuaSprite('healthbarChaotix', 'healthbar/healthbarChaotix', 475, 570);
	addLuaSprite('healthbarChaotix', false)
	setObjectCamera('healthbarChaotix', "hud")	
	scaleLuaSprite('healthbarChaotix', 1.1, 1.2);
	setObjectOrder('healthbarChaotix', 3)

	makeLuaSprite('barraHEALTH', '' , 1010, 640)
    makeGraphic('barraHEALTH', 22, 20,'000000')
    setProperty('barraHEALTH.visible', false);
    addLuaSprite('barraHEALTH', false)
    setObjectCamera('barraHEALTH', 'hud')
    setScrollFactor('barraHEALTH', 1, 1)
    setObjectOrder('barraHEALTH', 2)
...
if downscroll then
setProperty('healthbarChaotix.y', 9)
end
end
